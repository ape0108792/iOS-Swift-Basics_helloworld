...
class BranchListViewController: UITableViewController {
    ...
    @IBAction func locationTurnOn(_ sender: Any) {
        let locationAlert = UIAlertController(title:"위치정보요청", message: "위치정보를 기반으로 가까운 지점을 자동으로 선택할 수 있습니다.", preferredStyle: .actionSheet)
        
        let locationAction = UIAlertAction(title: "위치정보켜기", style: .default, handler: {(action:UIAlertAction) -> Void in print("위치정보켜기 선택")})
        
        let openMapAction = UIAlertAction(title: "지도에서 열기", style: .default, handler: {(action:UIAlertAction) -> Void in print("지도앱에서 열기 선택")})
